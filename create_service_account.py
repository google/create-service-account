...
"""GCP Cloud Shell script to automate creation of a service account for Google's
GWM, GWMME or Password Sync tools.

This script streamlines tool installation by automating the steps
...
TOOLS = {
        'GWM': {
            'FRIENDLY_NAME': 'Google Workspace Migrate',
            'NEEDS_WEB_CLIENT': True,
            'HELP_CENTER_URL': 'https://support.google.com/workspacemigrate/answer/10839762',
            'APIS': [
        ...
        'GWMME': {
            'FRIENDLY_NAME': 'Google Workspace Migration for Microsoft Exchange',
            'NEEDS_WEB_CLIENT': False,
            'HELP_CENTER_URL': 'https://support.google.com/a/answer/6291304',
            'APIS': [
                # If admin.googleapis.com is to be included, then it must be the first in
                # this list.
                "admin.googleapis.com",
        ...
        'PASSWORDSYNC': {
            'FRIENDLY_NAME': 'Password Sync',
            'NEEDS_WEB_CLIENT': False,
            'HELP_CENTER_URL': 'https://support.google.com/a/answer/7040511',
            'APIS': [
                    # If admin.googleapis.com is to be included, then it must be the first in
                    # this list.
                    "admin.googleapis.com",
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Create Service Account Script')
    parser.add_argument('--tool',
           help='Google tool to configure project and service account for.',
           required=True,
           dest='tool',
           type=str.upper,
           choices=TOOLS.keys())
...
async def create_project():
  ...
  project_id = f"{args.tool.lower()}-{int(time.time() * 1000)}"
  # project name is limited to 30 chars
  project_name = (f"{args.tool}-"
...
async def verify_tos_accepted():
  ...
  tos_accepted = False
  while TOOLS[args.tool]["APIS"] and not tos_accepted:
    command = f"gcloud services enable {TOOLS[args.tool]['APIS'][0]}"
...
async def enable_apis():
  ...
  enable_api_calls = map(enable_api, TOOLS[args.tool]["APIS"][1:])
...
async def create_service_account():
  ...
  service_account_name = f"{args.tool.lower()}-service-account"
  service_account_display_name = f"{args.tool} Service Account"
...
async def authorize_service_account():
  ...
  scopes = urllib.parse.quote(",".join(TOOLS[args.tool]["SCOPES"]), safe="")
  authorize_url = DWD_URL_FORMAT.format(service_account_id, scopes)
  input(f"\nBefore using {TOOLS[args.tool]['FRIENDLY_NAME']}, you must authorize the service "
...
async def verify_service_account_authorization():
  ...
  while not scopes_are_authorized:
    ...
    for scope in TOOLS[args.tool]['SCOPES']:
...
async def verify_api_access():
  ...
  token = get_access_token_for_scopes(admin_user_email, TOOLS[args.tool]['SCOPES'])
  retry_api_verification = True
  while retry_api_verification:
    disabled_apis = {}
    disabled_services = []
    retry_api_verification = False
    for api in TOOLS[args.tool]['APIS']:
...
def execute_api_request(url, token):
  try:
    ...
    headers = {
        ...
        "User-Agent": f'{args.tool}_create_user_account_v{VERSION}', 
    }
...
def init_logger():
  ...
  logging.basicConfig(
      filename=f"{args.tool}_create_service_account.log",
...
async def main():
  global args, KEY_FILE
  args = parse_args()
  KEY_FILE = (f"{pathlib.Path.home()}/{args.tool.lower()}-service-account-key-"
            f"{datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.json")
  init_logger()
  os.system("clear")
  friendly_name = TOOLS[args.tool]['FRIENDLY_NAME']
  response = input(
      "Welcome! This script will create and authorize the resources that are "
      f"necessary to use {friendly_name} ({args.tool}). The following steps will be "
      ...
      f"then you can follow the instructions at {TOOLS[args.tool]['HELP_CENTER_URL']}{ZWSP}."
  ...
  project_id = await get_project_id()
  if TOOLS[args.tool].get('NEEDS_WEB_CLIENT'):
